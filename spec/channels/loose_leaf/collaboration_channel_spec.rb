...
module LooseLeaf
  RSpec.describe CollaborationChannel do
    ...
    it 'should inform a client of current attribute versions when subscribing to a document' do
      ...
      expect(channel).to receive(:transmit).with(
    ...
    it 'should broadcast new operations' do
      ...
      expect_any_instance_of(ActionCable::Server::Broadcasting).to(
        receive(:broadcast)
        .with(
          "loose_leaf.documents.#{example_document.id}.operations",
          'version' => 1,
          'document_id' => example_document.id,
          'attribute' => 'body',
          'operation' => ['test'],
